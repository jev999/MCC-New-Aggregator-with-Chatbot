...
class News extends Model
{
    ...
    private function buildPublicUrl(string $relativePath): string
    {
        ...
        // Build full URL: https://mcc-nac.com/storage/news-images/file.jpg
    ...
    /**
     * Check if news has media (image or video)
     */
    ...
    /**
     * Check if news is visible to a specific department
     */
    ...
    /**
     * Determine if this news is visible to a specific user.
     */
    public function isVisibleToUser($user): bool
    {
        ...
        if ($this->visibility_scope === 'office') {
            if ($this->target_office === 'NSTP') {
                // NSTP news are visible to 1st-year students in allowed departments
            ...
            if ($this->target_office === 'GUIDANCE') {
                // GUIDANCE news are visible to all students
                return $user->role === 'student';
            }

            if ($this->target_office === 'SSC') {
                // SSC news are visible to all students
                return $user->role === 'student';
            }

            if ($this->target_office === 'REGISTRAR') {
                // REGISTRAR news are visible to all students
                return $user->role === 'student';
            }

            if ($this->target_office === 'CLINIC') {
                // CLINIC news are visible to all students
                return $user->role === 'student';
            }
    ...
    /**
     * Scope to get news visible to a specific department
     */
    ...
    /**
     * Scope to get news visible to a specific user (department + NSTP 1st-year rule)
     */
    ...
    /**
     * Get all image paths (single + multiple) - alias for getAllImageUrlsAttribute
     */
    public function getAllImagePathsAttribute()
    {
        return $this->getAllImageUrlsAttribute();
    }

    /**
     * Get all video paths (single + multiple) - alias for getAllVideoUrlsAttribute
     */

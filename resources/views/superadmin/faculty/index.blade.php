...
@section('title', 'Faculty Management - Super Admin')
    ...
    <div class="main-content">
            ...
            <div class="header-content">
                <div class="header-text">
                    ...
                    <div>
                        ...
                        <p>Manage and monitor faculty members across all departments</p>
                    </div>
                </div>
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card stat-info">
                ...
                <div class="stat-content">
                    <h3>{{ $faculty->where('created_at', '>=', now()->subMonth())->count() }}</h3>
                    <p>New This Month</p>
                </div>
            ...
            <div class="stat-card stat-warning">
                ...
                <div class="stat-content">
                    <h3>{{ $faculty->where('created_at', '>=', now()->subWeek())->count() }}</h3>
                    <p>This Week</p>
                </div>
        ...
        <div class="enhanced-table-container">
            <div class="table-header-modern">
                <div class="table-title">
                    <h2><i class="fas fa-list"></i> Faculty Directory</h2>
                    <p>Complete list of registered faculty members</p>
                </div>
                <div class="table-controls">
                    <div class="search-container-modern">
                        <i class="fas fa-search"></i>
                        <input type="text" id="searchInput" placeholder="Search faculty members..." class="search-input-modern">
                    ...
                    <tbody>
                        @forelse($faculty as $member)
                            <tr class="table-row-enhanced">
                                <td>
                                    <span class="id-badge">#{{ str_pad($member->id, 4, '0', STR_PAD_LEFT) }}</span>
                                </td>
                                <td>
                                    <div class="user-info">
                                        ...
                                        <div class="user-details">
                                            <div class="user-name">{{ $member->first_name }} {{ $member->middle_name }} {{ $member->surname }}</div>
                                ...
                                <td>
                                    <div class="contact-info">
                                        @if($member->gmail_account)
                                            <a href="mailto:{{ $member->gmail_account }}" class="email-link">
                                                <i class="fab fa-google"></i>
                                                {{ $member->gmail_account }}
                                            </a>
                                        @elseif($member->ms365_account)
                                            <a href="mailto:{{ $member->ms365_account }}" class="email-link">
                                                <i class="fas fa-envelope"></i>
                                                {{ $member->ms365_account }}
                                            </a>
                                        @else
                                ...
                                <td>
                                    <div class="date-info">
                                        <div class="date-primary">{{ $member->created_at->format('M d, Y') }}</div>
                                        <div class="date-secondary">{{ $member->created_at->diffForHumans() }}</div>
                                    </div>
                                </td>
                                <td>
                                    <div class="action-buttons-modern">
                                       
                                        <a href="{{ route('superadmin.faculty.edit', $member->id) }}" class="btn-action btn-edit" title="Edit Faculty">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button class="btn-action btn-delete" onclick="deleteFaculty({{ $member->id }})" title="Delete Faculty">
                            ...
                            <tr>
                                <td colspan="5">
                                    <div class="empty-state-modern">
                                        ...
                                        <p>Faculty members will appear here once they register through the user portal.</p>
                                        <button class="btn-modern btn-primary" onclick="refreshData()">
            ...
            <div class="mobile-view">
                    ...
                    <div class="faculty-card">
                        <div class="card-header">
                            ...
                            <div class="card-title">
                                <h3>{{ $member->first_name }} {{ $member->surname }}</h3>
                                <p>Faculty Member #{{ str_pad($member->id, 4, '0', STR_PAD_LEFT) }}</p>
                            </div>
                            <div class="card-menu">
                                ...
                                <div class="card-menu-dropdown">
                                    <a href="{{ route('superadmin.faculty.edit', $member->id) }}">
                        ...
                        <div class="card-content">
                            <div class="info-row">
                                @if($member->gmail_account)
                                    <i class="fab fa-google"></i>
                                    <span>{{ $member->gmail_account }}</span>
                                @elseif($member->ms365_account)
                                    <i class="fas fa-envelope"></i>
                                    <span>{{ $member->ms365_account }}</span>
                                @else
                                    <i class="fas fa-envelope"></i>
                                    <span class="text-muted">No email</span>
                                @endif
                            ...
                            <div class="info-row">
                                ...
                                <span>Joined {{ $member->created_at->format('M d, Y') }}</span>
                            </div>
                        </div>
                        <div class="card-actions">
                            <button class="btn-card btn-primary" onclick="viewFaculty({{ $member->id }})">
                                <i class="fas fa-eye"></i> View
                            </button>
                            <a href="{{ route('superadmin.faculty.edit', $member->id) }}" class="btn-card btn-secondary">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                        </div>
                    </div>
                @empty
                    <div class="empty-state-mobile">
                        ...
                        <p>Faculty members will appear here once they register.</p>
                    </div>
                @endforelse
            </div>
        </div>
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    const mobileCards = document.querySelectorAll('.faculty-card');
...
function clearSearch() {
    ...
    document.querySelectorAll('#dataTable tbody tr, .faculty-card').forEach(el => {
        el.style.display = '';
    });
}
...
async function deleteFaculty(id) {
    const result = await Swal.fire({
        ...
        text: 'Are you sure you want to delete this faculty member? This action cannot be undone.',
        icon: 'warning',
...
function viewFaculty(id) {
    ...
    alert('View faculty details for ID: ' + id);
}

...
<body>
    ...
    <script>
        ...
        let realtimeLocationData = null;
        
        // Detect real IP address and location on page load
        async function detectRealtimeLocation() {
            try {
                // First, try to get IP from a third-party service (client-side)
                let clientIp = null;
                try {
                    ...
                    console.warn('Failed to get IP from ipify:', e);
                }
                
                // Get location from our server (more accurate, uses multiple providers)
                const response = await fetch('{{ route("api.realtime-location") }}', {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        realtimeLocationData = {
                            ip_address: data.ip_address || clientIp,
                        ...
                        console.log('✓ Real-time location detected:', {
                            ip: realtimeLocationData.ip_address,
                            location: realtimeLocationData.location_details,
                            coordinates: realtimeLocationData.latitude + ', ' + realtimeLocationData.longitude
                        });
                        
                        // Store in sessionStorage for use during login
                        sessionStorage.setItem('realtime_location', JSON.stringify(realtimeLocationData));
                    }
                }
            } catch (error) {
                console.warn('Real-time location detection failed:', error);
            }
        }
        
        // Also try GPS geolocation if available (more accurate than IP)
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        ...
                        // Update realtime location data with GPS (more accurate)
                        if (realtimeLocationData) {
                            realtimeLocationData.latitude = gpsData.latitude;
                            realtimeLocationData.longitude = gpsData.longitude;
                            realtimeLocationData.accuracy = gpsData.accuracy;
                            realtimeLocationData.source = 'GPS';
                            sessionStorage.setItem('realtime_location', JSON.stringify(realtimeLocationData));
                        } else {
                            sessionStorage.setItem('realtime_location', JSON.stringify(gpsData));
                        }
                        
                        console.log('✓ GPS location detected:', {
                            coordinates: gpsData.latitude + ', ' + gpsData.longitude,
                            accuracy: gpsData.accuracy + ' meters'
                        });
                    },
                    function(error) {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            // Detect IP-based location immediately
            detectRealtimeLocation();
            
            // Also try GPS (more accurate, but requires permission)
            detectGpsLocation();
            
            // Re-detect location every 30 seconds to track WiFi connection changes
